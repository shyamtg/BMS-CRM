...
#     filter_class = UpcomingAppointmentsFilter
#
#
...
#         appointments = models.Appointments.objects.filter(user_id=self.request.user.id)
...
class AppointmentsViewSet(UserListViewSet, viewsets.ModelViewSet):
    ...
    def paginate_queryset(self, queryset):
        if 'no_page' in self.request.query_params:
            return None
        return super(AppointmentsViewSet, self).paginate_queryset(queryset)
...
class ClientUserMapViewSet(viewsets.ModelViewSet):
    ...
    def paginate_queryset(self, queryset):
        if 'no_page' in self.request.query_params:
            return None
        return super(ClientUserMapViewSet, self).paginate_queryset(queryset)
